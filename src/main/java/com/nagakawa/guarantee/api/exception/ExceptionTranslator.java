...
public class ExceptionTranslator implements ProblemHandling, SecurityAdviceTrait {
	...
	public ResponseEntity<Problem> process(@Nullable ResponseEntity<Problem> entity, NativeWebRequest request) {
		...
		ProblemBuilder builder = Problem.builder()
				.withType(Problem.DEFAULT_TYPE.equals(problem.getType()) ? ErrorConstants.Type.DEFAULT_TYPE
						: problem.getType())
		...
		if (problem instanceof ConstraintViolationProblem) {
			builder.with(VIOLATIONS_KEY, ((ConstraintViolationProblem) problem).getViolations()).with(MESSAGE_KEY,
					ErrorConstants.Key.ERR_VALIDATION);
		} else {
	...
	public ResponseEntity<Problem> handleMethodArgumentNotValid(MethodArgumentNotValidException ex,
		...
		Problem problem = Problem.builder().withType(ErrorConstants.Type.CONSTRAINT_VIOLATION_TYPE)
				.withTitle("Method argument not valid").withStatus(defaultConstraintViolationStatus())
				.with(MESSAGE_KEY, ErrorConstants.Key.ERR_VALIDATION).with(FIELD_ERRORS_KEY, fieldErrors).build();
	...
	public ResponseEntity<Problem> handleBadRequestAlertException(BadRequestAlertException ex,
		...
		return create(ex, request, HeaderUtil.createFailureAlert(true, ex.getEntityName(),
	...
	public ResponseEntity<Problem> handleConcurrencyFailure(ConcurrencyFailureException ex, NativeWebRequest request) {
		Problem problem = Problem.builder().withStatus(Status.CONFLICT)
				.with(MESSAGE_KEY, ErrorConstants.Key.ERR_CONCURRENCY_FAILURE).build();
